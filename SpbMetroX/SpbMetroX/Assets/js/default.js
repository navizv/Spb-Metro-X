...
function find_best_way(way, sc, s2, best) {
    ...
    if(way.dir == -1 || way.dir == 0 || way.dir == 2) {
        if (way.cs > 0) {
            ...
            way.way.push(sc2);
            wtime = way.time;
            if (way.dir == 0 || way.dir == 2) { //if we just get to the station
                wtime = way.time + 3;//wait for train
            }
            best = find_best_way({ time: wtime + lt[way.cl][way.cs - 1], way: way.way, dir: -1, cl: way.cl, cs: way.cs - 1 }, sc2, s2, best);
            way.way.pop();
        }
    }
    ...
    if (way.dir == 1 || way.dir == 0 || way.dir == 2) {
        if (way.cs < l[way.cl].length) {
            ...
            best = find_best_way({ time: wtime + lt[way.cl][way.cs], way: way.way, dir: 1, cl: way.cl, cs: way.cs + 1 }, sc2, s2, best);
            way.way.pop();
        }
    }
    ...
    if (way.dir == 1 || way.dir == 0 || way.dir == -1) {
        for (var p in per) {
            ...
            if (i >= 0 && way.way.indexOf(per[p][1 - i]) < 0) {
                ...
                way.way.push(sc2);
                best = find_best_way({ time: way.time + pert[p], way: way.way, dir: 2, cl: csns.line, cs: csns.station }, sc2, s2, best);
                way.way.pop();
            }
        }
    }
